...
- **做空优先级**：
  - 一级做空信号：4h/1h均线空头排列 + 15m出现超买 + 资金费率>0.01%
  - 二级做空信号：价格反弹至MA60上方但量能不足 + RSI背离
  - 三级做空信号：窄幅震荡后向下突破 + 成交量配合
- **做多限制条件**（必须同时满足）：
...
reason 必须包含：多周期趋势判断、熊市环境下的方向选择依据、仓位计算过程。
