...
- **持仓期间操作限制**：
  - 亏损时：禁止主动平仓/减仓，等待止损单自动触发
  - 盈利时：价格正向波动 ≥ 1% 时才允许提前平仓/减仓
  - 加仓：任何时候都可以加仓（遵守1/20规则）
  - 移动止损：盈利时可以移动止损保护利润
...
- 开仓价格: ____
- 当前价格: ____
- 价格正向波动: ____% （计算方法：多头=(现价-开仓价)/开仓价，空头=(开仓价-现价)/开仓价）
- **操作限制**：
  - 若正向波动 < 1%：禁止平仓/减仓，只能加仓或observe
  - 若正向波动 ≥ 1%：可以平仓/减仓/移动止损
  - 若亏损（正向波动<0）：禁止平仓/减仓，等待止损单触发
