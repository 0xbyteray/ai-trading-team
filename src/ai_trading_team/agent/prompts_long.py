...
- **大小周期结合**：4h定大方向（看多），1h确认趋势延续，15m/5m找精准入场点
...
- **频繁交易惩罚机制**：
  ...
  - 冷静期内仓位规模减半，且只允许顺大周期趋势方向操作（当前环境=只做多）
...
- **做多优先级**：
  - 一级做多信号：4h/1h均线多头排列 + 15m出现超卖 + 资金费率<0.01% → 重仓做多（杠杆15-20x）
  - 二级做多信号：价格回调至MA60下方但量能萎缩 + RSI背离 → 中仓做多（杠杆10-15x）
  - 三级做多信号：窄幅震荡后向上突破 + 成交量配合 → 轻仓做多（杠杆5-10x）
- **做空限制条件**（必须同时满足）：
  - 1) 价格突破近期高点后明显滞涨（明确超买）
  - 2) RSI(15m) > 75 且 RSI(1h) > 70
  - 3) 资金费率 > 0.05%（多头过度拥挤）
  - 4) 4h级别出现明显顶部结构（双顶/射击之星等）
  - 5) 仅博回调3-5%，杠杆不超过10x，严格止盈
- **趋势跟随策略**：
  - 在上涨趋势中，优先在回调支撑位做多
  - 关注关键支撑位的守护情况，跌破支撑需谨慎
...
reason 必须包含：多周期判断、牛市环境下的方向选择依据、手续费成本计算、与近期操作的价格距离检查、仓位计算；开仓/加仓必须给出 take_profit_price 并确保盈亏比≥1:4。
"""
...
## 你的任务
根据当前市场行情和牛市环境，设置一个合理的止损价格来保护利润。
...
### 建议的止损距离参考：
- 10%盈利: 可设置在成本价或保护5%利润（做空单保护8%）
- 20%盈利: 可设置保护10-15%利润（做空单保护15%）
- 30%盈利: 可设置保护20-25%利润（做空单保护25%）
...
{{
    ...
    "reason": "<详细解释你选择这个止损价格的理由，包括考虑了哪些技术因素、牛市环境下的特殊考量、以及为何这个位置能平衡保护利润与避免过早止损>"
}}
"""
